...
var initialDb = {
  settings: {
    id: "1",
    companyName: "Lifevision Healthcare",
    logoText: "Lifevision Healthcare",
    email: "info@lifevisionhealthcare.com",
    phone: "+91-9878845222",
    address: "Plot No. 140, Industrial Area, Phase 1, Panchkula, Haryana, India - 134113",
    whatsappNumber: "919878845222",
    facebookUrl: "https://facebook.com/lifevisionhealthcare",
    twitterUrl: "https://twitter.com/lifevisionhc",
    linkedinUrl: "https://linkedin.com/company/lifevisionhealthcare",
    instagramUrl: "https://instagram.com/lifevisionhealthcare",
    youtubeUrl: "https://youtube.com/c/lifevisionhealthcare",
    metaTitle: "WHO-GMP Certified Third Party Pharma Manufacturing | Lifevision Healthcare",
    metaDescription: "Lifevision Healthcare is a leading ISO, WHO, GMP certified pharmaceutical company specializing in third party manufacturing, contract manufacturing, and PCD pharma franchise business in India.",
    metaKeywords: "Pharma third party manufacturing, pharma franchise, WHO-GMP pharma company, Lifevision Healthcare, pharma contract manufacturing",
...
function readDb() {
  try {
    if (!import_fs.default.existsSync(dbPath)) {
      import_fs.default.writeFileSync(dbPath, JSON.stringify(initialDb, null, 2));
      return initialDb;
    }
    const data = import_fs.default.readFileSync(dbPath, "utf-8");
    const parsed = JSON.parse(data);
    if (!parsed.newLaunches) {
      parsed.newLaunches = initialDb.newLaunches;
      import_fs.default.writeFileSync(dbPath, JSON.stringify(parsed, null, 2));
    }
    return parsed;
  } catch (err) {
...
function writeDb(data) {
  try {
    import_fs.default.writeFileSync(dbPath, JSON.stringify(data, null, 2));
    return true;
  } catch (err) {
...
function addActivityLog(user, action, ipAddress = "127.0.0.1") {
  const db = readDb();
  const newLog = {
    id: `log_${Date.now()}`,
    user,
...
app.post("/api/system/backup", (req, res) => {
  const db = readDb();
  const backupFilename = `lifevision_backup_${(/* @__PURE__ */ new Date()).toISOString().replace(/[:.]/g, "-")}.json`;
  const backupDir = import_path.default.join(process.cwd(), "server", "backups");
  if (!import_fs.default.existsSync(backupDir)) {
    import_fs.default.mkdirSync(backupDir, { recursive: true });
  }
  import_fs.default.writeFileSync(import_path.default.join(backupDir, backupFilename), JSON.stringify(db, null, 2));
